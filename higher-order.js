...
// JASIMINE.JS IS CHECKING AN ALGORITHYM WITH FAULTY MATH ON THIS PROBLEM!!!
...
let orderTotals = orders.map((element)=> {
  result= element.price += element.price*element.tax
  result1 = result += result*element.tax
  return result1
 })
...
let bobsTotal = purchases.filter(element =>element.owner == 'Bob').map(element => element.price)
